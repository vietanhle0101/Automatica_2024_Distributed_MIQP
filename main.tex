...
Let consider an optimization problem involving $N$ agents.
...
It is obvious that $\Phi^{(t)}$ is bounded below by $0$.
In what follows, we prove that $\Phi^{(t)}$
...
\end{pmatrix}
\end{equation}
is positive definite, $\{\Phi^{(t)}\}$ is a monotonically non-increasing sequence.
\end{lemma}
