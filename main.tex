...
Throughout the paper, we will utilize the following notations.
...
Let $\bbsym{x}_i$ with a dimension of $n_i$ be the vector of local optimization variables of \agent{i}.
...
<<<<<<< HEAD
We consider that $\bbsym{Q}_i$ is positive definite so that $f_i$ is a convex quadratic function.
=======
Thus, the coupling constraint can be also represented by
${\bbsym{A} \bbsym{x} = \bbsym{b}}$.
>>>>>>> 05737c3fe5e66c79ee28b21a95d5d4ed519b6bbf
...
\begin{assumption}
The problem \eqref{eq:miopt-prb} is feasible.
\end{assumption}

We define the relaxed problem for the original problem \eqref{eq:miopt-prb} as follows
\begin{equation}
\label{eq:rlx-prb}
\begin{split}
\underset{\bbsym{x}_i \in \tilde{\XXX}_i}{\min} & \quad \sum_{i=1}^N \; f_i (\bbsym{x}_i), \\
\text{subject to} & \quad \sum_{i=1}^N \bbsym{A}_{i} \bbsym{x}_i = \bbsym{b}, \\
\end{split}
\end{equation}
where $\tilde{\XXX}_i$ is formed from $\XXX_i^{\rm{MI}}$ by relaxing the integrality constraints.

Since the relaxed problem \eqref{eq:rlx-prb} is a convex QP and it is also feasible, there exists a saddle point $(\bbsym{x}^{*}, \bbsym{\lambda}^{*})$ that satisfies the KKT conditions, \ie there exists $\bbsym{\nu}_i^* \in N_{\tilde{\XXX}_i} (\bbsym{x}_i^{*})$ such that
\begin{equation}
\begin{split}
& \nabla f_i(\bbsym{x}_i^{*}) + \bbsym{A}_i^\top \bbsym{\lambda}^{*} =- \bbsym{\nu}_i^*,\; \forall i = 1, \dots, N, \\
...
\section{Distributed Mixed-Integer Optimization with Proximal ADMM and Sequential Convex Programming}

\subsection{Algorithm Design}

To solve the MIQP problem, we combine proximal ADMM \cite{yang2022proximal,deng2017parallel} with sequential convexification of the integrality constraints as presented follows.
At each iteration $t$, we keep a mixed-integer-valued vector $\bbsym{x}_i^{(t)} \in \XXX_i^{\rm{MI}}$ and a real-valued solution of the relaxed problem $\tilde{\bbsym{x}}_i^{(t)} \in \tilde{\XXX}_i$.
...
\item Compute $\tilde{\bbsym{x}}_i^{(t+1)}$ from $\bbsym{x}_i^{(t+1)}$ by rounding operator (transform a real-valued solution into a mixed-integer one). 
In other words,
\begin{equation}
...
\Note{Include algorithm pseudo-code.}
...
\Note{Discuss how to apply the algorithm to decentralized setting.}
...
Thus, in what follows, we prove the e the sufficiently decreasing property of $\Phi^{(t)}$.
...
\begin{proof}
The optimality condition of \eqref{eq:local_update} implies that there exists $\bbsym{\nu}_i^{(t+1)}$ such that 
\begin{equation}
\begin{multlined}
\bbsym{A}_i^\top \Big( \bbsym{\lambda}^{(t)} + \rho \big( \bbsym{A}_i \bbsym{x}^{(t+1)} - \bbsym{b} + \sum_{j \neq i} \bbsym{A}_j \xtil_j^{(t)} \big) \Big) \\
+ \beta_i (\bbsym{x}^{(t+1)} - \xtil_i^{(t)} ) + \nabla f_i (\bbsym{x}^{(t+1)}) = - \bbsym{\nu}_i^{(t+1)}
\end{multlined}
\end{equation}
...
\Note{Can we also prove the the converged point is a stationary point?}
...
Our future work will consider the algorithms for solving mixed-integer nonlinear programming. 
